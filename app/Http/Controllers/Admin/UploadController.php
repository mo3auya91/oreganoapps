...
//                    'upload_url' => $this->get_full_url() . $this->thumb_upload_path,
        ...
        //dd($this->options);
        if ($initialize) {
            //$this->initialize($title,$status);
            $this->initialize();
        }

    }

    //protected function initialize($title,$status)
...
//                $this->post($title,$status,$this->options['print_response']);
                break;
            case 'DELETE':
    ...
    protected function jquery_validate($uploaded_file, $file, $error, $index)
    {
        if ($error) {
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error, $index = null, $content_range = null)
    {
        ...
        $file->name = Str::random(50) . '.' . $ext;
        $file->size = $this->fix_integer_overflow((int)$size);
        $file->type = $type;
        if ($this->jquery_validate($uploaded_file, $file, $error, $index)) {
